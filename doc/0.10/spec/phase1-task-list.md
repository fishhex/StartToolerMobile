...
- [ ] 添加 `go_router`、`provider`、`http`、`flutter_secure_storage` 到 pubspec.yaml（T2 立即用到 secure storage，提前加）
- [ ] 5 个路由：`/splash`、`/connect`（含 `/connect/manual` 子）、`/home`（含 `/home/project-switcher` 子）、`/settings`
- [ ] 路由守卫：splash 1.5s 后跳 `/connect`；当前空间非空时 `/connect` 系列重定向到 `/home`
...
- [ ] `ConnectView`（`/connect`）— 「粘贴 QR 内容」文本框 + 「解析并连接」按钮 + 「手动输入 IP」入口
- [ ] `ManualInputView`（`/connect/manual`）— IP + Port + secret 三个 `TextField` + 「连接」按钮（调 health 验证）
...
- [ ] `HomeView` 接收 `host` / `port` 参数（M1 路由参数跳转；T2 改为从持久化读）
- [ ] `initState` 调 `health_api.check` → 顶栏显示 `name`
- [ ] 健康成功后才允许进入主页

### T1 验收

- [ ] 启动 App → 1.5s splash → `/connect`
- [ ] 粘贴合法 QR 内容 → 解析 → 调 health → 跳 `/home`，顶栏显示 PC 名
- [ ] 粘贴非法 QR → 顶部 banner 提示「二维码无效」
...
- [ ] 错误映射：复用 T1.4 的网络/超时/4xx/5xx，额外 401 → `AuthError.expired`
...
- [ ] 首次扫码 → 健康 → 跳主页 → 主页显示真实 projects 列表
- [ ] 杀掉 App 重新启动 → 自动 health 验证 → 直接进主页（不需再扫码）
...
2. 4 个 commit（每个 task 一个）：
   - `feat(flutter): T1 骨架 — 路由 + 4 页面 + QR 解析 + health 客户端`
...
按 T1 → T2 → T3 → T4 顺序执行；每个 task 内 commit 可细分（如 T1 内拆「加依赖」「QR 解析」「health 客户端」「页面骨架」等）。